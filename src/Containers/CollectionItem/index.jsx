...
const Todo = ({collectionName}) => {
  ...
  return (
    <>
      <>
        <CreateTodo collectionName={collectionName}/>
        <TodoContainer collectionName={collectionName}>
...
const Item = ({ item, collectionName }) => {
  ...
  return (
    <ItemContainer key={item.id}>
      <EditToDo collectionName={collectionName} isEdit={isEdit} setIsEdit={setIsEdit} updateToDo={updateToDo} docId={item.id}/>
      <div  className="container1">
        <div>{item.title}</div>
        <div className="container2">
          <div>
      ...
      {open ? <div style={{backgroundColor: '#567', marginBottom: '20px'}}><AddItem collectionName={collectionName} docRef={docRef} docId={item.id} open={open}/></div> : null}
    </ItemContainer>
  );
};
